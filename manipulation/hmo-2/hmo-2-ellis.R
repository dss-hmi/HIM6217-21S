...
path_db  <- "data-public/derived/hmo_2.sqlite3"

path_patients <- "./data-public/raw/hmo-2/patient.csv"
path_dx <- "./data-public/raw/hmo-2/dx.csv"
path_visits <- "./data-public/raw/hmo-2/visit.csv"
...
col_types_visits <- 
  readr::cols_only(
    ...
    `visit_category`        = readr::col_character()
)

col_types_dx <- 
  readr::cols_only(
    `dx_id`     = readr::col_integer(),
    `person_id`   = readr::col_integer(),
    `dx_date`   = readr::col_date(),
    `icd9_code`        = readr::col_character(),
    `icd9_label`        = readr::col_character()
  )
...
ds_dx       <- readr::read_csv(path_dx , col_types=col_types_dx)
...
sql_create <- c(
  ...
  CREATE TABLE `dx` (
    ...
    icd9_label       varchar(255)  not null
  );
  "
  ,
  "
  ...
  "
    CREATE TABLE visit (
      ...
      person_id       INTEGER   not null,
      dx_date         DATE      not null,
      visit_category  varchar(30)  not null
  "
)
...
DBI::dbWriteTable(cnn, name='dx',   value=ds_dx_slim,    append=TRUE, row.names=FALSE)
